...
namespace laboratorna5
{
    class GraduateStudet : Person, IDateAndCopy
    {
        ...
        {
            return $"\n Name: {PersonProperty.Name}\n Last Name: {PersonProperty.LastName}\n" +
        ...
        internal class YearComparer : IComparer<GraduateStudet>
        {
            public int Compare(GraduateStudet x, GraduateStudet y)
            {
                if (x.LearningYear.CompareTo(y.LearningYear) != 0)
                {
                    return x.LearningYear.CompareTo(y.LearningYear);
                }
                else
                {
                    return 0;
